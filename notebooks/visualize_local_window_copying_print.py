...
DEFAULT_EXPERIMENT = S4_ROOT / "configs" / "experiment" / "synthetic" / "s4-local-window-copying-v3.yaml"
...
def plot_example(inputs: torch.Tensor, targets: torch.Tensor, config: dict[str, Any]) -> plt.Figure:
    ...
    # Use binary markers only for visualization; keep original ids for window/query lookup.
    plot_markers = np.zeros_like(markers)
    plot_markers[markers > 0] = 1.0
    plot_markers[markers < 0] = -1.0
    ...
    fig, axes = plt.subplots(2, 1, figsize=(6, 6), constrained_layout=True, sharex=False)

    # Input signal with markers
    axes[0].plot(time, signal, color="#1f78b4", linewidth=1.2, label="Signal")
    axes[0].step(time, plot_markers, where="mid", color="#d95f02", alpha=0.7, label="Window Marker")
    ...
    for i, (offset, window_id) in enumerate(queries):
        axes[0].axvline(query_start + offset, color="#e31a1c", linestyle="--", alpha=0.8, label="Copy prompt" if i == 0 else None)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to save the generated figure (defaults to ./local_window_copying.png).",
    )
    parser.add_argument("--no-plot", action="store_true", help="Skip matplotlib visualization (still saves).")
...
def main() -> None:
    ...
    save_path = args.save or (Path.cwd() / "local_window_copying.png")
    save_path = save_path if save_path.is_absolute() else Path.cwd() / save_path
